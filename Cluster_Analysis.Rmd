...
# Map each cluster to the Type label it contains most often (majority vote),
# then compare the mapped labels against the actual Type variable.

kmap <- sapply(sort(unique(kmc3f$cluster)), function(k)
          names(which.max(table(data$Type[kmc3f$cluster == k]))))
km_pred <- factor(kmap[kmc3f$cluster], levels = levels(factor(data$Type)))
confusionMatrix(km_pred, factor(data$Type))
```
...
# Map each HCA group to the Type label it contains most often (majority vote),
# then compare the mapped labels against the actual Type variable.

hmap <- sapply(sort(unique(group)), function(k)
          names(which.max(table(data$Type[group == k]))))
hca_pred <- factor(hmap[group], levels = levels(factor(data$Type)))
confusionMatrix(hca_pred, factor(data$Type))
```
